...
public:
  ...
  void attachLongPress(callbackFunction newFunction); // called whenever a button has been pressed for longer than longPress_time
...
private:
  ...
  int _longPressTime;       // number of milli seconds that have to pass  before we consider a long press to have occured
  bool _long_press_registered; // helps to ensure only one long press until button is released
